...
# Premium Custom Emoji IDs disabled as per request to keep it standard for everyone
PREMIUM_EMOJI_IDS = {}

def premium_emoji(text):
    """Returns standard text with normal emojis to avoid broken tags for non-premium users"""
    return text
...
async def check_card(card, site, proxy):
    ...
    try:
        ...
        if not gate or gate.lower() == 'unknown' or gate.lower() == 'shopiii':
...
async def send_realtime_hit(user_id, result, hit_type, username):
    ...
    if not gateway_name or gateway_name.lower() == 'unknown' or gateway_name.lower() == 'shopiii':
...
<blockquote>🌐 𝐆𝐚𝐭𝐞𝐰𝐚𝐲: 🔥 {gateway_name} | 💰 {result.get('price', '-')}</blockquote>
<b>━━━━━━━━━━━━━━━━━</b>
...
async def update_progress(user_id, message_id, results, current_attempt_count):
    ...
    if not gateway or gateway.lower() == 'unknown' or gateway.lower() == 'shopiii':
...
🤖 <b>Bot By: <a href="tg://user?id=5248903529">ㅤㅤＫａｍａｌ</a></b>"""
    ...
    async with aiofiles.open(filename, 'w') as f:
        ...
        for r in results['charged']:
            await f.write(f"{r['card']} | {r.get('gateway', 'Shopify Payments')} | {r.get('price', '-')} | {r['message'][:100]} | {r.get('site', 'Unknown')}\n")
        await f.write("\n")
...
async def proxy_command(event):
    ...
    try:
        ...
        summary_msg = "✅ <b>Proxy Check Complete!</b>\n\n"
...
async def site_command(event):
    ...
    try:
        ...
        summary_msg = "✅ **Site Check Complete!**\n\n"
